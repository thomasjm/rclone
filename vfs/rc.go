...
attributes for the refreshed entries, so out-of-band changes are
seen at once; other mount backends (cmount, NFS) can't and will
wait for --attr-timeout.
` + getVFSHelp,
	})
}
...
On a mount or mount2 (Linux) this also drops the kernel's cached
attributes so an out-of-band change is seen at once; other mount
backends (cmount, NFS) can't and will wait for --attr-timeout.
` + getVFSHelp,
	})
}
